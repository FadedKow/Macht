...
async def nsfw(ctx):
    ...
    embed.add_field(name=f"`{prefix}nsfw_avatar`", value=f"*Random NSFW avatar*", inline=False)
    embed.set_thumbnail(url="https://i.imgur.com/pVbTpks.png")
    embed.set_footer(icon_url="https://i.imgur.com/pVbTpks.png",text=" Macht Selfbot - Made by Founder#8300")
    try:
      await ctx.send(embed=embed)
...
async def nsfw_avatar(ctx):
    ...
    embed = discord.Embed(colour=hexColor, title="NSFW Avatar",description="Random NSFW avatar by nekos.life lib")
